...
{
    public async Task<OrchestratorStartResponse?> StartAsync(IReadOnlyList<(string Name, byte[] Bytes)> files, string description)
    {
        ...
        var response = await http.PostAsync("api/ai/start", content);
    ...
    public async Task<GraphDto?> GenerateFinalBrainMapAsync(GenerateFinalBrainMapRequest request)
    {
        var response = await http.PostAsJsonAsync("api/ai/generate-final-brain-map", request);
        return response.IsSuccessStatusCode ? await response.Content.ReadFromJsonAsync<GraphDto>() : null;
    }
}
